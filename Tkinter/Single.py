...
class single_room:
	def cetak (self):
		...
		self.konfimasi = Button( master , text = 'Konfirmasi' , command = self.konfimasi ).grid(row=9, column=1)
	...
	def single_booking(self):
		...
		self.input_hari = Entry(master , text = self.hari ).grid(row=9, column=1)
